...
export type ToolResult={summary:string;data:unknown;citations?:{title:string;url:string}[];actionProposal?:{id:string;description:string;type:'submit_request'};actions?:SuggestedAction[]};
...
export async function executeTool(name:string,args:unknown,session:Session):Promise<ToolResult>{
 ...
 const {assessment,guidance}=assessWithGuidance(input.dates,input.leaveCode,requests);
 if(name==='find_alternative_dates'){const options=alternativeLeaveDates(input.dates,input.leaveCode,requests);return{summary:`Found ${options.length} alternative date blocks.`,data:{options,limitation:'Ranked by recorded booked staff-days; not certified safe staffing.'},actions:options.map((o,i)=>({id:'alternative-'+i,label:o.dates[0]+' · '+o.hours+'h',kind:'prompt' as const,prompt:`Check ${input.leaveCode} leave on ${o.dates.join(', ')}.`}))}}
 ...
 if(name==='check_leave')return{summary:`${assessment.hours}h requested; ${assessment.canProceed?'manager review required':'request needs an update'}.`,data:{...assessment,guidance},actions:guidance.actions,citations:policies.documents.filter(d=>['anf-2024','ratios','accrued-leave'].includes(d.id)).map(d=>({title:d.title,url:d.pdfUrl||d.url}))};
