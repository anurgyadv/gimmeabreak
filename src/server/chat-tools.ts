...
import type {Workforce,PolicyIndex} from '../lib/workforce-types';
...
import {listBookings} from './chat-store';
...
export async function executeTool(name:string,args:unknown,session:Session):Promise<ToolResult>{
 ...
 const assessment=assessWithRequests(workforce,policies,input.dates,input.leaveCode,requests);
 ...
 if(name==='check_leave')return{summary:`${assessment.hours}h requested; ${assessment.canProceed?'manager review required':'request needs an update'}.`,data:assessment,citations:policies.documents.filter(d=>['anf-2024','ratios','accrued-leave'].includes(d.id)).map(d=>({title:d.title,url:d.pdfUrl||d.url}))};
 if(name==='find_shift_options'){const options=findAvailableSwaps(workforce,policies,input.dates,requests).slice(0,5);return{summary:`Found ${options.length} possible arrangements; clinical suitability remains unverified.`,data:{scope:'Connected nursing candidate pool: same-role/grade staff in department plus up to50 compatible staff. Not an exhaustive whole-archive search.',options,limitations:['Colleague availability is inferred from recorded schedules, not consent.','No message has been sent.','Cross-ward skills and patient staffing requirements require manager confirmation.']}}}
 if(!assessment.canProceed)return{summary:'This request cannot be submitted yet.',data:{...assessment,message:'Resolve failed checks or overlapping requests before confirmation.'}};
