...
export async function executeTool(name:string,args:unknown,session:Session):Promise<ToolResult>{
 ...
 if(name==='get_my_requests'){objectArgs(args,[]);const rows=await listBookings(employeeId);return{summary:`Found ${rows.length} saved requests.`,data:rows.map(r=>({id:r.id,dates:r.dates,leaveType:r.leaveType,status:r.status,managerNote:r.managerNote,submittedAt:r.submittedAt,hours:r.assessment.hours,alternativeDates:r.alternativeDates,swapResponse:r.swapInvitation?{status:r.swapInvitation.status,reason:r.swapInvitation.reason}:null}))}}
