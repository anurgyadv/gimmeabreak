...
const localFile=()=>process.env.CHAT_LOCAL_STORE||path.join(process.cwd(),'.release-local','chat-state.json');
...
export async function listInvitations(employeeId:string):Promise<LeaveInvitation[]>{
 ...
 const rows:LeaveInvitation[]=[];for await(const e of t.listEntities<{payload:string}>({queryOptions:{filter:`PartitionKey eq 'invitations-${employeeId}'`}}))rows.push(JSON.parse(e.payload));return rows.sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export async function saveInvitation(invitation:LeaveInvitation){
 ...
 await t.upsertEntity({partitionKey:'invitations-'+invitation.employeeId,rowKey:invitation.id,payload:JSON.stringify(invitation)},'Replace');return invitation;
}
