...
export function guidanceForAssessment(assessment:Assessment,dates:string[],absences:BookedAbsenceCount[],leaveCode='AL'){
 ...
 reasons.push(...assessment.checks.filter(c=>c.status==='warning'&&['mix','policy'].includes(c.id)).map(c=>c.detail));
 ...
 return{status:assessment.canProceed?'review' as const:'changes' as const,title:assessment.canProceed?'Ready for manager review':'Your request needs changes',subtitle:assessment.canProceed?'Checks complete. Your manager still needs to approve this request.':'Resolve these blockers before submitting.',reasons,bookedAbsences:booked,actions};
}
