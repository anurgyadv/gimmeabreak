...
import {getStaffingFloors,staffingImpact} from '@/server/staffing-plan';
...
export async function PATCH(req:Request){try{sameOrigin(req);const s=requireSession(req);if(s.role!=='manager')return Response.json({message:'Unlock the assistant with the manager access code to save a central decision.'},{status:403});return await withEmployeeLock(s.employeeId,async()=>{const body=await readBody(req,3000),b=(await listBookings(s.employeeId)).find(r=>r.id===body.id);if(!b||b.status!=='manager-review')throw new Error('Request is not awaiting review.');if(!['approved','declined','changes-requested'].includes(body.status))throw new Error('Invalid decision.');const note=typeof body.note==='string'?body.note.trim().slice(0,1000):'';if(body.status==='approved'){if(b.swap&&b.swapInvitation?.status!=='accepted')throw new Error('Confirm clinical verification and record colleague acceptance before approving this swap.');if(!canApprove(b,body.verified===true))throw new Error('Confirm clinical verification before approving.');const other=(await listBookings(s.employeeId)).filter(r=>r.id!==b.id);if(b.swap&&!findAvailableSwaps(raw as Workforce,policyRaw as unknown as PolicyIndex,b.dates,other).some(o=>o.id===b.swap?.id))throw new Error('The request no longer has an available swap. Recheck the roster.');const impact=staffingImpact(b.dates,other,await getStaffingFloors(),b.swap);if(impact.some(x=>x.shortfall!==null&&x.shortfall>0))throw new Error('The request no longer meets the configured staffing minimum. Arrange cover or choose other dates.');}if(body.status!=='approved'&&!note)throw new Error('A decision reason is required.');const at=new Date().toISOString(),updated={...b,status:body.status,managerNote:note,events:[...b.events,{at,label:body.status,detail:note||'Manager confirmed clinical verification.'}]};await saveBooking(updated);await audit(s.employeeId,'manager-decision',{id:b.id,status:body.status,note});return Response.json({ok:true,request:updated})})}catch(e){return Response.json({message:safeMessage(e)},{status:400})}}
